    ...
    <%@ page import="dev.sgvero.confbsas.sgv.OradorDAO" %>
        <%@ page import="dev.sgvero.confbsas.sgv.Orador" %>
            ...
            <body>
                ...
                <% // Obtener el ID del orador desde el parámetro de la URL String idStr=request.getParameter("id"); int
                    id=Integer.parseInt(idStr); // Obtener el orador actual desde la base de datos OradorDAO
                    oradorDAO=new OradorDAO(); Orador orador=oradorDAO.getOradorById(id); if (orador !=null) { %>
                    <span class="link" id="form"></span>
                    <section class="container mt-5 formulario">
                        <h6>modifica los datos del orador</h6>
                        <h3>"<%= orador.getNombre()%>
                                <%= orador.getApellido() %>"</h3>
                        <%-- Mostrar la foto actual --%>
                            <div class="editar-foto">
                                <img id="vista-previa" src="<%= orador.getFotoPath() %>" alt="Vista previa de la foto"
